class Api::V1::UsersController < ActionController::API
  ...
  def related_account_status
    ...
    if user
      ...
      response_message[:code] = 200
      response_message[:message] = "sorry, the user does not exist"
    end
    render json: response_message
  end
  ...
  def returning
    ...
    loan.update(end_time: Time.now)
    income = Income.create(book_id: book.id, begin_at: loan.start_time, end_at: loan.end_time, amount: fees)
    response_message[:code] = 200
